...
def generate_one_completion(prompt):
    ...
    outputs = language_model.generate(input_ids = token_inputs['input_ids'].to(CUDA),
                                      ...
                                      temperature=0.8, num_return_sequences=5, max_new_tokens = 256,
                                      output_logits = True, 
...
def seq2text(prompt, outputs_sequences):
    ...
    return [clean(s) for s in answer]
...
tasks = pickle.load(open('interrogate_tasks.pickle', 'rb'))
...
for idx in trange(len(tasks)):
    ...
    task_num, subtasks = tasks[idx]

    seed = 1
    all_code = []

    for prompt, correct_prompt in subtasks:
        set_seed(seed)
        print(task_num, seed)
        #print('-------------PROMPT-------------')
        #print(prompt)
        ...
        all_code.append(code_clean)
        #print(code_clean)
        #pickle.dump(code_clean, open('/code_clean.pickle', 'wb'))
        #sys.exit()
        ...
        #results_code[task_id] = ([c[prompt_len_chars:] for c in code_clean], outputs.sequences.cpu().clone(), probs)

        del outputs
        torch.cuda.empty_cache()

    pickle.dump(all_code, open('/inter_llm/%d.pickle' % task_num, 'wb'))
